...
- 项目目录：`F:\服务\api-pool`
- 管理面板：`http://127.0.0.1:5100/login`
- 客户端 Base URL：`http://127.0.0.1:5100/v1`
...
- 已验证：`py_compile` 通过；重启 5100 服务后完整流程通过——登录、拉取可用模型、锁定后 `/v1/models` 仅剩该模型、`api_config.json` 持久化、解除后恢复未锁定。
