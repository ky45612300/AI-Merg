...
class APIPool:
    ...
    def _active_endpoints(self, requested_model=None):
        ...
        locked = (self.locked_model or "").strip()
        requested = (requested_model or "").strip()
        target_model = locked or requested
        if target_model:
    ...
    def chat(self, messages, model=None, extra_payload=None, timeout=None, return_endpoint=False):
        ...
        locked_model = (self.locked_model or "").strip()
        if locked_model and requested_model and requested_model != locked_model:
            raise ModelRouteError(
                requested_model,
                f"服务当前锁定模型为 {locked_model}，不能请求 {requested_model}",
                status=400,
                error_type="model_locked",
            )

        target_model = locked_model or requested_model
...
def load_locked_model():
    try:
        with open(CONFIG_FILE, "r", encoding="utf-8") as f:
            val = json.load(f).get("locked_model")
...
def save_config(endpoints_data, locked_model=None, station_settings=None, model_aliases=None):
    data = {"api_endpoints": endpoints_data}
    if locked_model:
        data["locked_model"] = locked_model
    if station_settings:
...
pool.locked_model = load_locked_model()
...
def list_openai_models():
    now = int(time.time())
    if pool.locked_model:
        return {"object": "list", "data": [{"id": pool.locked_model, "object": "model", "created": now, "owned_by": "api-pool"}]}
    seen = set()
    models = []
    with pool._lock:
...
def make_chat_completion_response(body, result):
    if isinstance(result, dict):
        if pool.locked_model:
            result["model"] = pool.locked_model
        return result
    return {
        "id": f"chatcmpl-{int(time.time()*1000)}",
        "object": "chat.completion",
        "created": int(time.time()),
        "model": pool.locked_model or body.get("model") or "api-pool-aggregated",
...
def make_responses_response(body, text, served_model=None, tool_calls=None):
    ...
    return {
        ...
        "created_at": created,
        "status": "completed",
        "model": pool.locked_model or served_model or body.get("model") or "api-pool-aggregated",
        "output": output,
        "output_text": text,
...
def api_handler(method, path, body):
        ...
        try:
            ...
            if is_stream:
                def compat_stream():
                    ...
                    model = pool.locked_model or model or body.get("model") or "api-pool-aggregated"
                    message = {}
                    finish_reason = "stop"
                    if isinstance(result, dict):
                        try:
    ...
    if method == "GET" and cp == "/api/locked-model":
        seen = set(); models = []
        for ep in pool.list_endpoints():
        ...
        return 200, {"locked_model": pool.locked_model or "", "available_models": models}, False
    if method == "POST" and cp == "/api/locked-model":
        val = (body.get("locked_model") or "").strip()
        pool.locked_model = val or None
        pool._manual_override_id = None
        pool._current_idx = 0
        _sync_to_config()
        sys_log(f"锁定模型已{'设置为 ' + pool.locked_model if pool.locked_model else '解除'}", "INFO")
        return 200, {"ok": True, "locked_model": pool.locked_model or ""}, False
...
def _sync_to_config():
    save_config([{"id": ep.get("id"), "name": ep["name"], "base_url": ep["base_url"], "api_key": ep.get("api_key_full", ep.get("api_key", "")), "model": ep["model"], "public_model": ep.get("public_model", ep["model"]), "upstream_model": ep.get("upstream_model", ep["model"]), "priority": ep["priority"], "timeout": ep["timeout"], "max_retries": ep["max_retries"], "enabled": ep["enabled"], "cooldown_minutes": ep["cooldown_minutes"], "daily_limit": ep.get("daily_limit", 0), "rpm_limit": ep.get("rpm_limit", 0), "use_proxy": ep.get("use_proxy", True), "protocol": ep.get("protocol", "openai"), "health_mode": ep.get("health_mode", "chat"), "billing_mode": ep.get("billing_mode", "subscription"), "is_vision": ep.get("is_vision", True), "in_pool": ep.get("in_pool", True)} for ep in pool.list_endpoints()], locked_model=pool.locked_model, station_settings=pool.station_settings, model_aliases=pool.model_aliases)
...
<div class="api-info-card">
  <div style="font-size: 13px; font-weight: 700; color: var(--accent-light); margin-bottom: 10px; text-transform: uppercase; letter-spacing: 0.5px;">🔒 锁定模型 (Fixed Model)</div>
  <div style="display: flex; gap: 12px; flex-wrap: wrap; align-items: center; font-size: 13px;">
    <span style="color: var(--text-dim);">当前状态:</span>
    <code id="lockedModelState">加载中</code>
    <select id="lockedModelSelect" style="padding:6px 10px;border-radius:8px;border:1px solid rgba(255,255,255,.15);background:rgba(0,0,0,.25);color:#fff;outline:none;font-size:13px;min-width:200px;">
      <option value="">— 不锁定（聚合所有模型）—</option>
    </select>
    <button class="key-action-btn" onclick="setLockedModel()" title="锁定所选模型">应用</button>
    <button class="key-action-btn copy" onclick="clearLockedModel()" title="解除锁定">解除</button>
  </div>
  <div style="font-size:11px;color:var(--text-dim);margin-top:8px;">锁定后，外部应用无论请求什么模型，都只会用该模型回复；同名模型的多个上游仍会自动故障转移。</div>
</div>
...
<div class="api-info-card">
  ...
  <div id="aliasList" style="display:flex;flex-direction:column;gap:8px;font-size:13px;"></div>
...
async function loadLockedModel(){
  const r=await api('GET','/api/locked-model');
  if(!r||r.ok===false)return;
  const sel=document.getElementById('lockedModelSelect');
  const state=document.getElementById('lockedModelState');
  const cur=r.locked_model||'';
  const opts=['<option value="">— 不锁定（聚合所有模型）—</option>'];
  (r.available_models||[]).forEach(m=>{
    const tag=m.enabled&&m.in_pool?'':' (未启用/池外)';
    opts.push(`<option value="${m.model}"${m.model===cur?' selected':''}>${m.model}${tag}</option>`);
  });
  if(cur&&!(r.available_models||[]).some(m=>m.model===cur))
    opts.push(`<option value="${cur}" selected>${cur} (无对应端点!)</option>`);
  sel.innerHTML=opts.join('');
  if(cur){state.textContent='🔒 已锁定：'+cur;state.style.color='var(--accent-light)';}
  else{state.textContent='未锁定（聚合模式）';state.style.color='var(--text-dim)';}
}
async function setLockedModel(){
  const v=document.getElementById('lockedModelSelect').value;
  const r=await api('POST','/api/locked-model',{locked_model:v});
  if(r&&r.ok){toast(v?('已锁定：'+r.locked_model):'已解除锁定','success');loadLockedModel();refresh();}
  else toast('设置失败','error');
}
async function clearLockedModel(){
  const r=await api('POST','/api/locked-model',{locked_model:''});
  if(r&&r.ok){toast('已解除锁定','success');loadLockedModel();refresh();}
}
...
async function loadModelAliases(){
  ...
  if(!entries.length){list.innerHTML='<div style="color:var(--text-dim);">暂无映射</div>';}
  else{
    list.innerHTML=entries.map(([a,t])=>`<div style="display:flex;gap:10px;align-items:center;">
      <code>${esc(a)}</code><span style="color:var(--text-dim);">→</span><code>${esc(t)}</code>
      <button class="key-action-btn" onclick="removeModelAlias('${esc(a)}')" title="删除该映射" style="color:var(--red);">删除</button>
    </div>`).join('');
  }
